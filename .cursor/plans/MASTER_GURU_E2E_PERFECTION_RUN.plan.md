...
| 2   | Run quality gates                                     | `npm run quality` (format:check, lint, test:unit)                                       | Done                              |
| 3   | Run pipeline verification                             | `npm run verify:pipeline` (or `npm run verify:pipeline:strict` when creds are expected) | Done (runbook cred-gated)         |
