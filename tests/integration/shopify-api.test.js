...
import { resolveShopifyStoreInfo } from '../../src/shopify/store-domain.js';
...
describe('Shopify API Integration', () => {
  ...
  const adminHost = resolveShopifyStoreInfo(storeDomain).adminHost;
  ...
  itIf(hasCredentials)('should connect to Shopify API', async () => {
    const response = await fetch(`https://${adminHost}/admin/api/2026-01/shop.json`, {
      headers: {
  ...
  itIf(hasCredentials)('should fetch products from Shopify', async () => {
    const response = await fetch(`https://${adminHost}/admin/api/2026-01/products.json?limit=1`, {
      headers: {
        'X-Shopify-Access-Token': accessToken,
        'Content-Type': 'application/json',
      },
  ...
  itIf(hasCredentials)('should fetch themes from Shopify', async () => {
    const response = await fetch(`https://${adminHost}/admin/api/2026-01/themes.json`, {
      headers: {
        'X-Shopify-Access-Token': accessToken,
        'Content-Type': 'application/json',
      },
