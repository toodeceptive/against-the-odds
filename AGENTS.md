...
- **`python` symlink**: The Playwright config references `python` (not `python3`). On Linux where only `python3` exists, create a symlink: `sudo ln -sf /usr/bin/python3 /usr/local/bin/python`. The update script handles this automatically.
- **E2E `example.spec.js` failures**: Two tests hit the remote GitHub Pages URL (`toodeceptive.github.io`) which may return "Site not found" — this is an external hosting issue, not a local setup problem. The `storefront.spec.js` tests that use `localhost:8080` pass.
