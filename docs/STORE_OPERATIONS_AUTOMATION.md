...
| **Themes (update, edit files, deploy)**               | Repo + GitHub App     | Edit `src/shopify/themes/aodrop-theme/`, preview with `theme-dev.ps1`, then commit/push to `main` so CI updates `shopify-theme`            | Fallback CLI: `update-theme.ps1` / `theme-update-store.ps1`; E2E: Admin      |
| **Content (theme copy, pages, homepage)**             | Edit files + GitHub   | Edit files in `src/shopify/themes/aodrop-theme/`, preview with `theme-dev.ps1`, then commit/push to `main`                                 | Fallback CLI: `theme-update-store.ps1`; E2E: Theme editor in Admin           |
...
- **Edit locally:** Change Liquid/sections/snippets/templates in that folder; preview with `theme-dev.ps1`, then commit/push to `main` so CI updates `shopify-theme`.
...
- **Theme copy:** Edit files in `src/shopify/themes/aodrop-theme/` (sections, templates, snippets); preview locally, then commit/push to `main`.
