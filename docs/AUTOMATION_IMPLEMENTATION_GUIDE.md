...
| **Theme update → preview → save**             | **GitHub flow + optional capture**        | Preferred chain: edit theme → preview → approve → commit/push to `main` → CI updates `shopify-theme`. Direct CLI push remains a fallback for explicit direct-apply cases only.                                                | Manual: run theme-dev, check in browser, run update-theme.             |
...
3. **Save:** Preferred path is commit/push to `main` so CI updates `shopify-theme`. Use `.\scripts\shopify\update-theme.ps1` only when you intentionally need a direct CLI push after preview + approval.
