...
| **SHOPIFY_CLI_THEME_TOKEN** | **Theme access**: Run `npx shopify auth login`, then create a theme token in Shopify Admin → Apps → Develop apps → Create an app → Theme library → Create theme access token. Put the token in `.env.local` as `SHOPIFY_CLI_THEME_TOKEN`. | Or rely on interactive Shopify CLI login when previewing/pushing themes.                                                                                                                                                                                                                 | **Required for seamless preview**: so `scripts/open-preview-popup.ps1` can start theme dev without prompting for login. |
