...
5. **Approval**: Wait for explicit user confirmation in chat (e.g. "approve", "yes", "looks good").
6. **Apply**: Commit and push to `main`. If theme-branch CI is configured, it will update `shopify-theme`; Shopify deploys from the connected branch. Alternatively, run `node scripts/shared/run-powershell.cjs scripts/shopify/update-theme.ps1` for direct CLI push (still requires approval first).
...
| **Apply after approval (theme)**    | Run `node scripts/shared/run-powershell.cjs scripts/shopify/update-theme.ps1`; append to [deploy-log.md](status/deploy-log.md); clear [pending-approval.md](status/pending-approval.md) (write "No pending changes").                                                                                                          |
