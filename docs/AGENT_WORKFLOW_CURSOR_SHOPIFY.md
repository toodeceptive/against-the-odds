...
- **Snapshot before approval**: Before asking for approval, write the preview (dry-run output or theme URL + diff) to [docs/status/pending-approval.md](status/pending-approval.md). For **product** changes, run `.\scripts\open-pending-approval.ps1`. For **theme** changes, run `.\scripts\open-preview-popup.ps1` (or `.\scripts\start-theme-preview.ps1`) so the approval file opens in Cursor and the preview opens in the browser (static mock + live theme when server is ready). After apply, clear the file (write "No pending changes" placeholder).
...
| **Open pending approval**           | Tasks → **Open pending approval** or **Ctrl+Alt+P** (opens docs/status/pending-approval.md). After writing the file, agent runs `.\scripts\open-pending-approval.ps1` for product-only changes.                                                                              |
