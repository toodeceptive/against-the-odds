...
- note: Credentials closed via save-token-from-file.ps1 (client credentials flow). Structural signature regenerated via WSL bash; ssh-keygen verify PASS. Branch protection aligned (arch_guard, quality, secret-scan, test). GH_TOKEN fallback added to verify-secrets. Remaining: GitHub Actions repo secrets (SHOPIFY_*), optional gh auth login for persistent secrets visibility.
