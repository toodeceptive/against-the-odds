...
- **GitHub workflows:** `.github/workflows/` — ci.yml (consolidated quality gate), codeql.yml (CodeQL analysis; continue-on-error), shopify-sync.yml, sync-theme-branch.yml, sync.yml, maintenance.yml. They trigger on **main**; no develop branch in use.
