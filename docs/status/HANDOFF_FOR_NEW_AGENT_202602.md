...
**Prompt for new agent:** Use **`docs/status/PLAN_AGENT_ENTRY.md`** and **`prompts/PERFECT_EXECUTION_PROMPT.md`** to start; for full takeover use **`docs/AGENT_PROMPT_DECISION_TREE.md`** and the canonical plan (`.cursor/plans/FINAL_REPO_ORGANIZATION_AND_AUDIT.plan.md`).
...
- **GitHub workflows:** `.github/workflows/` — ci.yml (consolidated quality gate), shopify-sync.yml, sync-theme-branch.yml, sync.yml, maintenance.yml. They trigger on **main**; no develop branch in use.
