...
**Follow-up (2026-02-26)**: Added `scripts/github/set-secrets-from-env.ps1` to push .env.local to repo secrets; made SHOPIFY_THEME_ID optional in verify-secrets per workflows README; documented in OPERATOR_RUNBOOK, CREDENTIAL_SETUP_STEPS, Executive Summary. Optimized set-secrets to use GITHUB_TOKEN from .env.local when gh needs auth. Remaining: run `set-secrets-from-env.ps1` (requires approval per shopify-preview-approval) to enable shopify-sync in CI.
