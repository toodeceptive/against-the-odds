...
| **Update store theme (preferred)**        | Preview, approve, then commit/push to `main` so CI updates `shopify-theme` |
| **Direct-apply helper (legacy/explicit)** | `.\scripts\shopify\theme-update-store.ps1`                                 |
...
**Store not showing brand?** First confirm the default flow is working: push approved theme changes to `main` and let CI update `shopify-theme`. Use `theme-update-store.ps1` only as an explicit direct-apply helper after preview + approval.
...
| 5. **Deploy theme**  | Commit and push approved changes to `main`                           | Preferred path: CI updates `shopify-theme`, and Shopify deploys from that connected branch.                                                                                    |
| 6. **Direct apply**  | `.\scripts\shopify\theme-update-store.ps1`                           | Legacy/explicit direct-apply helper. Requires preview + approval first; use when intentionally bypassing the GitHub branch flow.                                               |
