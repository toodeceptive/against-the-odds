...
| **Deploy via GitHub App (canonical)**   | Commit/push to `main` so CI updates `shopify-theme`                 |
...
**Store not showing brand?** First confirm the GitHub-App path is healthy: commit/push to `main`, then verify `sync-theme-branch.yml` updated `shopify-theme`. Use `theme-update-store.ps1` only as a direct CLI fallback when the GitHub deploy path is unavailable.
...
| 5. **Commit repo**   | Commit and push to `main`                                            | Canonical theme deploy path; CI subtree-syncs `src/shopify/themes/aodrop-theme` to `shopify-theme`.                                                                            |
| 6. **Deploy theme**  | Shopify GitHub App deploys from `shopify-theme`                      | Preferred store apply path. Use `theme-update-store.ps1` only as a fallback direct CLI push when the GitHub-App path is unavailable.                                           |
